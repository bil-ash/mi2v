...
from diffusion.model.nets.rope import RoPE3D, PositionGetter3D
...
class LiteLA(Attention_):
    ...
    def forward(self, x: torch.Tensor, mask=None, HW=None, block_id=None) -> torch.Tensor:
        ...
        #print("dim",self.dim)
        q = q.transpose(-1, -2)  # (B, h, h_d, N) -> (B, h, N, h_d)
        #k = k.transpose(-1, -2)  # (B, h, h_d, N) -> (B, h, N, h_d)
        ...
        if N==2640:
            ...
            pos_thw = self.position_getter(B, t=3, h=16, w=16, device=q.device)
            q = self.rope(q, pos_thw)
            k = self.rope(k, pos_thw)
        else:
            ...
            q = self.rope(q, pos_thw)
            k = self.rope(k, pos_thw)
        #pos_thw = self.position_getter(B, t=3, h=90, w=160, device=q.device)

        q = q.transpose(-1, -2)  # (B, h, N, h_d) -> (B, h, h_d, N)
        #k = k.transpose(-1, -2)  # (B, h, N, h_d) -> (B, h, h_d, N)
...
class Attention(Attention_):
    ...
    def forward(self, x, HW=None):
        ...
        q = self.q_norm(q)
        k = self.k_norm(k)
        q = q.reshape(B, N, H, C // H).permute(0, 2, 1, 3)  # (B, H, N, C//H)
        k = k.reshape(B, N, H, C // H).permute(0, 2, 1, 3)  # (B, H, N, C//H)
...
class PatchEmbed_3D(nn.Module):
    ...
    def __init__(
        ...
        self.num_patches = self.grid_size[0] * self.grid_size[1] *17
...
class LinearMappingNetwork(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Linear(43200, 2760)  # 将43200维映射到2760维

    def forward(self, x):
        x = x.permute(0, 2, 1)  # 将输入的形状从 [1, 43200, 1152] 转换为 [1, 1152, 43200]
        x = self.linear(x)      # 应用线性层
        x = x.permute(0, 2, 1)  # 将形状转回 [1, 2760, 1152]
        return x
...
class TokenDownsample(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=2, stride=2, padding=0):
        super(TokenDownsample, self).__init__()
        # 定义卷积层，用于下采样
        self.conv = nn.Conv2d(
        ...
        self.relu = nn.ReLU()  # 激活函数（可选）

    def forward(self, x):
        # 输入 x 的形状为 [batch_size, tokens, dim]
        # 将 tokens 分解为 T, H, W
        batch_size, tokens, dim = x.shape
        #print(tokens)
        T, H, W = 3, 22, 40
        assert tokens == T * H * W, "Tokens 不匹配"
        
        # 调整形状为 [batch_size, T, H, W, dim]
        x = x.view(batch_size, T, H, W, dim)
        
        # 转换为 [batch_size * T, dim, H, W]，以适配 Conv2D
        x = x.permute(0, 1, 4, 2, 3).contiguous().view(batch_size * T, dim, H, W)
        
        # 应用卷积下采样
        x = self.conv(x)
        x = self.relu(x)
        
        # 下采样后形状为 [batch_size * T, out_channels, H//2, W//2]
        # 转换回 [batch_size, T, H//2, W//2, out_channels]
        x = x.view(batch_size, T, H // 2, W // 2, dim).permute(0, 1, 2, 3, 4).contiguous()
        
        # 调整形状为 [batch_size, tokens', dim]
        tokens_downsampled = T * (H // 2) * (W // 2)
        x = x.view(batch_size, tokens_downsampled, dim)
        return x
...
class TokenUpsample(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=2, stride=2, padding=0):
        super(TokenUpsample, self).__init__()
        # 定义转置卷积层，用于上采样
    ...
    def forward(self, x):
        ...
        # 转换为 [batch_size * T, dim, H, W]，以适配 ConvTranspose2D
        x = x.permute(0, 1, 4, 2, 3).contiguous().view(batch_size * T, dim, H, W)
        ...
        # 上采样后形状为 [batch_size * T, out_channels, H*2, W*2]
        # 转换回 [batch_size, T, H*2, W*2, out_channels]
        x = x.view(batch_size, T, H * 2, W * 2, dim).permute(0, 1, 2, 3, 4).contiguous()
        
        # 调整形状为 [batch_size, tokens', dim]
        tokens_upsampled = T * (H * 2) * (W * 2)
        x = x.view(batch_size, tokens_upsampled, dim)

...
class SanaBlock_cross(nn.Module):
    ...
    def forward(self, x, y, t, flow_score, mask=None, H=None, W=None,T=None, S=None,**kwargs):
        ...
        if flow_score is not None:
            ...
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
                self.scale_shift_table[None] + t.reshape(B, N, 6, -1)
            ).chunk(6, dim=2)
        #print(shift_msa.shape,scale_msa.shape)
        shift_msa = shift_msa.squeeze(2)
        scale_msa = scale_msa.squeeze(2)
...
class Mobiledit(nn.Module):
    ...
    def forward(self, x, timestep, guide_image, y, cond_mask , flow_score, mask=None, data_info=None, **kwargs):
        ...
        timestep = timestep.unsqueeze(1).repeat(1, x.shape[1])
        ...
        if flow_score is not None:
            flow_score = flow_score.unsqueeze(1).repeat(1, x.shape[1])
...
def Mobiledit_300M_P1_D16(**kwargs):
    return Mobiledit(depth=16, hidden_size=1152, patch_size=1, num_heads=16, **kwargs)

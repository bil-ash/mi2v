...
#from mmenging.registry import Registry
...
def get_vae(name, model_path, device="cuda"):
    if name == "sdxl" or name == "sd3":
        ...
        return dc_ae
    elif "video-vae" in name:
        print(colored(f"[Video-vae] Loading model from {model_path}", attrs=["bold"]))
        video_vae = AutoencoderKLLTXVideo.from_pretrained(model_path).to(device).eval()
        return video_vae
    else:
...
def vae_encode(name, vae, images, sample_posterior, device):
    if name == "sdxl" or name == "sd3":
        ...
        z = z * scaling_factor
    elif "video-vae" or "finrtune_ltx" in name:
        ae = vae
        scaling_factor = ae.config.scaling_factor if ae.config.scaling_factor else 0.41407
        z = ae.encode(images.to(device)).latent_dist.sample()
        #z = (z - vae.latents_mean.to(z.dtype).view(1, -1, 1, 1, 1))/ vae.latents_std.to(z.dtype).view(1, -1, 1, 1, 1)
        z = z * scaling_factor
...
def vae_decode(name, vae, latent):
    if name == "sdxl" or name == "sd3":
        ...
        try:
            ...
            samples = ae.decode(latent / scaling_factor, return_dict=False)[0]
    elif name == "video-vae" or "finrtune_ltx":
        latent = (latent.detach() / vae.config.scaling_factor)
        #print(latent.shape)
        #latent = latent * vae.latents_std.to(latent.dtype).view(1, -1, 1, 1, 1) + vae.latents_mean.to(latent.dtype).view(1, -1, 1, 1, 1)
        #latent = latent[:,:128,:,:,:]
        samples = vae.decode(latent).sample
    else:
        print("error load vae")

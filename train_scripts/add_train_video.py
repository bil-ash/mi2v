...
import sys
sys.path.insert(0, ".")
from diffusion.data.datasets.sana_video_data import VideoTextDataset
#from diffusion.data.videodatasets.utils.parallel_states import get_data_parallel_group
#from diffusion.data.videodatasets.dataloader import prepare_dataloader
from torchvision.io import write_video
from einops import rearrange
from diffusion.model.addistill.discriminator import Discriminator
    ...
    def run_sampling(init_z=None, label_suffix="", vae=None, sampler="dpm-solver"):
        ...
        for prompt in validation_prompts:
            ...
            if sampler == "dpm-solver":
                ...
                guide_image = z[:, :, :1, :, :]
                #flow_score = torch.zeros(z.shape[0], device=z.device)
                denoised = flow_solver.sample(z, guide_image, flow_score, steps=1)
        ...
        for prompt, latent in zip(validation_prompts, latents):
            ...
            for i, sample in enumerate(samples):
                save_sample(
                    sample,
                    fps=24,
                    save_path=os.path.join("vis", str(step), prompt[:50]),
                    verbose=False
                )
...
def train(config, args, accelerator, model, teacher_model, dmd_model, discriminator, optimizer, lr_scheduler, 
          optimizer_disc, lr_scheduler_disc, optimizer_dmd, lr_scheduler_dmd, train_dataloader, train_diffusion, logger):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if load_vae_feat:
                ...
                with torch.no_grad():
                    with torch.amp.autocast(
                        ...
                        # z = rearrange(z, "(B D) C H W -> B C D H W", B=B, D=D)
                ...
                if "T5" in config.text_encoder.text_encoder_name:
                    ...
                    with torch.no_grad():
                        ...
                        null_caption_token = tokenizer(
                            "", max_length=config.text_encoder.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
                        ).to(accelerator.device)
                        null_caption_embs = text_encoder(null_caption_token.input_ids, null_caption_token.attention_mask)[0]
                        null_y = null_caption_embs.repeat(len(prompt), 1, 1)[:, None]
                        # teacher_data = batch["text_feat"]
                else:
                    print("error")
                    exit()
            ...
            with accelerator.accumulate(model) and accelerator.accumulate(discriminator):
                ...
                if accelerator.is_main_process:
                    log_buffer.update({k: v.mean().item() for k, v in loss_term.items() if k != "loss"})
                ...
                optimizer.step()
                optimizer_disc.step()
                if train_diffusion.iter_steps % 2 == 0:
                    optimizer_dmd.step()
                lr_scheduler.step()
                lr_scheduler_disc.step()
                lr_scheduler_dmd.step()
                accelerator.wait_for_everyone()
            ...
            if (step + 1) % config.train.log_interval == 0 or (step + 1) == 1:
                ...
                # by myself
                info += (f"iter_step: {train_diffusion.iter_steps}, ")
            ...
            if (train_diffusion.iter_steps + 1) % config.train.save_model_steps == 0:  
                accelerator.wait_for_everyone()
                if accelerator.is_main_process:
                    os.umask(0o000)
                    ...
                    # by myself
                    disc_saved_path = save_checkpoint(
                        osp.join(config.work_dir, "discriminator"),
                        epoch=epoch,
                        step=global_step,
                        model=accelerator.unwrap_model(discriminator),
                        optimizer=optimizer_disc,
                        lr_scheduler=lr_scheduler_disc,
                        # generator=generator,
                        # add_symlink=True,
                    )
                    dmd_saved_path = save_checkpoint(
                        osp.join(config.work_dir, "dmdmodel"),
                        epoch=epoch,
                        step=global_step,
                        model=accelerator.unwrap_model(dmd_model),
                        optimizer=optimizer_dmd,
                        lr_scheduler=lr_scheduler_dmd,
                        # generator=generator,
                        # add_symlink=True,
                    )
...
def main(cfg: SanaConfig) -> None:
    ...
    global max_length, validation_prompts, latent_height, latent_width, valid_prompt_embed_suffix, null_embed_path
    ...
    latent_height = int(config.model.image_height) // config.vae.vae_downsample_rate
    latent_width = int(config.model.image_width) // config.vae.vae_downsample_rate
    print(f"image_size: {image_size}, latent_height: {latent_height}, latent_width: {latent_width}")
    ...
    # by myself
    teacher_model_kwargs = model_kwargs.copy()
    teacher_model_kwargs["cfg_embed"] = False
    ...
    discriminator = Discriminator(color_dim, discriminator_feature_dim).train()
    ...
    # 3. build dataloader
    config.data.data_path = config.data.data_path if isinstance(config.data.data_path, list) else [config.data.data_path]
    config.data.data_path = [
        data if data.startswith(("https://", "http://", "gs://", "/", "~")) else osp.abspath(osp.expanduser(data))
        for data in config.data.data_path
    ]
    ...
    if config.model.multi_scale:
        drop_last = True
        uuid = hashlib.sha256("-".join(config.data.data_path).encode()).hexdigest()[:8]
        ...
        for i in config.data.data_path:
            cache_file += f"-{i}"
        cache_file += ".json"
    ...
    optimizer_disc = build_optimizer(discriminator, config.train.optimizer)
    optimizer_dmd = build_optimizer(dmd_model, config.train.optimizer)
    ...
    lr_scheduler_disc = build_lr_scheduler(config.train, optimizer_disc, train_dataloader, lr_scale_ratio)
    lr_scheduler_dmd = build_lr_scheduler(config.train, optimizer_dmd, train_dataloader, lr_scale_ratio)
    logger.warning(
    ...
    dmd_model = accelerator.prepare(dmd_model)
    discriminator = accelerator.prepare(discriminator)
    optimizer_disc, lr_scheduler_disc = accelerator.prepare(optimizer_disc, lr_scheduler_disc)
    optimizer_dmd, lr_scheduler_dmd = accelerator.prepare(optimizer_dmd, lr_scheduler_dmd)
    ...
    train(
        ...
        optimizer_dmd=optimizer_dmd,
        lr_scheduler_dmd=lr_scheduler_dmd,

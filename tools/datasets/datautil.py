...
def main(args):
    ...
    if args.claritymin is not None:
        assert "clarity" in data.columns
        data = data[data["clarity"] >= args.claritymin]
...
def parse_args():
    ...
    parser.add_argument("--claritymin", type=float, default=None, help="filter the dataset by minimum clarity score")
...
def get_output_path(args, input_name):
    ...
    if args.claritymin is not None:
        name += f"_claritymin{args.claritymin}"
    if args.flowmin is not None:

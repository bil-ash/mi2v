...
def set_env(seed=0, latent_size_w=256, latent_size_h=256):
    torch.manual_seed(seed)
    torch.set_grad_enabled(False)
    for _ in range(30):
        torch.randn(1, 4, latent_size_w, latent_size_h)
...
def save_sample(x, save_path=None, fps=8, normalize=True, value_range=(-1, 1), force_video=False, verbose=True):
    ...
    """
    assert x.ndim == 4
    if not os.path.isdir(os.path.join("test_video")):
        os.makedirs(os.path.join("test_video"))
    save_path += ".mp4"
    if normalize:
    ...
    #x = x-100
    print(save_path)
    if not os.path.exists(save_path.split('/')[-2]):
        os.makedirs(save_path.split('/')[-2])
    write_video(save_path, crop_x, fps=fps, video_codec="h264")
    if verbose:
...
def visualize(config, args, model, items, bs, sample_steps, cfg_scale, pag_scale=1.0):
    ...
    for chunk in tqdm(list(get_chunks(items, bs)), desc=tqdm_desc, unit="batch", position=args.gpu_id, leave=True):
        ...
        prompts, hw, ar = (
            [],
            torch.tensor([[args.image_height, args.image_width]], dtype=torch.float, device=device).repeat(bs, 1),
        ...
        if bs == 1:
            ...
            #print("prompt",prompt)
            image_path =prompt
            #prompt = "Change this image to video."
        ...
        text_time = time.time()
        print("The time of text_encoder:",text_time-start_time)

        # start sampling
        print( latent_size[0],latent_size[1])
        with torch.no_grad():
            n = len(prompts)
            z = torch.randn(
                n,
                config.vae.vae_latent_dim,
                config.data.num_frames // 8 +1,
                latent_size[0],
                latent_size[1],
                device=device,
                generator=generator,
            ...
            flow_score = torch.FloatTensor([config.flow_score]).to(device)
        ...
        print("The time of sampling:",sampling_time-text_time)

        samples = samples.to(torch.float32)
        print(samples.shape)
        B, _, D, _, _ = samples.shape
        #samples = rearrange(samples, "B C D H W -> (B D) C H W")
        decode_time1 = time.time()
        samples = vae_decode(config.vae.vae_type, vae, samples)
        decode_time2 = time.time()
        print("The time of decode:",decode_time2-sampling_time)
        print("The total time:",decode_time2-start_time)
        ...
        for i, sample in enumerate(samples):
                save_sample(
                    sample,
                    fps=17,
                    save_path=os.path.join(args.save_path, image_name.split(".")[-2]),
                    verbose=False
                )
...
def get_args():
    ...
    parser.add_argument("--flow_score", type=float, help="flow_score")
...
if __name__ == "__main__":
    ...
    args.image_height = config.model.image_height
    args.image_width = config.model.image_width
    args.image_size = (config.model.image_height, config.model.image_width)
    if args.custom_image_size:
        args.image_size = args.custom_image_size
        print(f"custom_image_size: {args.image_size}")

    set_env(args.seed, args.image_height // config.vae.vae_downsample_rate, args.image_width // config.vae.vae_downsample_rate)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    logger = get_root_logger()

    # only support fixed latent size currently
    latent_h = args.image_height // config.vae.vae_downsample_rate
    if(args.image_height % config.vae.vae_downsample_rate != 0):
        latent_h = args.image_height // config.vae.vae_downsample_rate +1
    latent_size = (latent_h, args.image_width // config.vae.vae_downsample_rate)
    ...
    sample_steps_dict = {"dpm-solver": 20, "sa-solver": 25, "flow_dpm-solver": 20, "flow_euler": 30}
    ...
    #vae = get_vae(config.vae.vae_type, config.vae.vae_pretrained, device).to(torch.float16)
